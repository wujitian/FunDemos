...
DEFINE_APP(ShadowMapExample, "Shadow Map Example")
...
void ShadowMapExample::Initialize(const char * title)
{
    ...
    {
        { GL_VERTEX_SHADER, "../shaders/shadowmap/shadowmap_shadow.vs.glsl" },
        { GL_FRAGMENT_SHADER, "../shaders/shadowmap/shadowmap_shadow.fs.glsl" },
        { GL_NONE }
    };
    ...
    model_view_projection_matrix = glGetUniformLocation(render_light_prog, "model_view_projection_matrix");
    
    // Create the program for rendering the scene from the viewer's position
    ShaderInfo scene_shaders[] =
    {
        { GL_VERTEX_SHADER, "../shaders/shadowmap/shadowmap_scene.vs.glsl" },
        { GL_FRAGMENT_SHADER, "../shaders/shadowmap/shadowmap_scene.fs.glsl" },
        { GL_NONE }
    };
...
void ShadowMapExample::Display(bool auto_redraw)
{
    ...
    mat4 scene_model_matrix = rotate(t * 10.0f, Y);
    mat4 scene_view_matrix = translate(0.0f, 0.0f, -5.0f);
    ...
    //glUniformMatrix4fv(model_view_projection_matrix, 1, GL_FALSE, 
    ...
    glEnable(GL_POLYGON_OFFSET_FILL);       // open polygon offset
