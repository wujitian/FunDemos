...
DEFINE_APP(TransformFeedbackExample, "Transform Feedback Example")
...
void TransformFeedbackExample::Initialize(const char * title)
{
    ...
    for (int i = 0; i < 2; i++)
    {
        ...
        glBufferData(GL_TRANSFORM_FEEDBACK_BUFFER, point_count * (sizeof(vmath::vec4) + sizeof(vmath::vec3)), 
        ...
        if (i == 0)
        {
            ...
            for (int j = 0; j < point_count; j++)
            {
                ...
                buffer[j].position = vmath::vec4(buffer[j].velocity + vmath::vec3(-0.5f, 5.5f, 0.0f), 1.0f);
        ...
        glVertexAttribPointer(1, 3, GL_FLOAT, GL_FALSE, sizeof(vmath::vec4) + sizeof(vmath::vec3), 
                                                (GLvoid *)sizeof(vmath::vec4));
...
void TransformFeedbackExample::Display(bool auto_redraw)
{
    ...
    vmath::mat4 model_matrix(vmath::scale(1.1f) *
                             vmath::rotate(t * 10.0f, 0.0f, 1.0f, 0.0f) *
                             vmath::rotate(t * 10.0f * 3.0f, 0.0f, 0.0f, 1.0f));
    ...
    if (t > q)
    {
        ...
        glUniform1f(time_step_loc, (t - q) * 1.0f);
    }
    q = t;
